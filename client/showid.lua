...
Citizen.CreateThread(function()
    TriggerServerEvent("atomik-showid:add-id")
    while true do
        Citizen.Wait(1)
        if IsControlPressed(0, Config.key) or forceDraw then
            local playerPed = PlayerPedId()
            if IsPedArmed(playerPed, 4) == 1 then
                TriggerEvent("QBCore:Notify", "Elinde silah varken id bakamazsın bro.", "error")
                Citizen.Wait(1000)
            else
...
RegisterNetEvent('atomik-showid:client:add-id')
AddEventHandler('atomik-showid:client:add-id', function(identifier, playerSource)
